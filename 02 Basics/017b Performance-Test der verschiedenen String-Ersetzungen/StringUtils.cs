...
namespace Addison_Wesley.Codebook.Basics
{
	public class StringUtils
	{
			...
			string replacement, bool ignoreCase, int start, int count)
		{
			...
			// Den Sonderfall abhandeln, das count mit 0 angegeben ist
			if (count == 0)
			{
				return source;
			}
			...
			// erfolgen soll, werden der zu durchsuende und der Suchstring
			// einfach in Kleinschreibung umgewandelt
			if (ignoreCase)
			{
			...
			while (pos2 != -1)
			{
				...
				if (findCount >= start)
				{
					// Das Ergebnis zusammensetzen und die Such-Indizes auf
					// die neuen Positionen setzen
			...
			{
				// Wenn nach dem letzten Suchstring noch ein Teilstring
				// gespeichert ist diesen hinten anhängen
